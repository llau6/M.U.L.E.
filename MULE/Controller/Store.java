...
public class Store implements Initializable {
    ...
    public void initialize(URL location
            , ResourceBundle resources) { //SPLIT THIS METHOD UP
        newMoney.setText(String.valueOf(
                GameManager.getCurrentPlayer()
                        .getMoney()));
        newFood.setText(String.valueOf(
                GameManager.getCurrentPlayer()
                        .getFoodCount()));
        newEnergy.setText(String.valueOf(
                GameManager.getCurrentPlayer()
                        .getEnergyCount()));
        newOre.setText(String.valueOf(
                GameManager.getCurrentPlayer()
                        .getOreCount()));
        foodQuantityLabel.setText(String.valueOf(
                StoreManager.getFoodQuantity()));
        energyQuantityLabel.setText(String.valueOf(
                StoreManager.getEnergyQuantity()));
        oreQuantityLabel.setText(String.valueOf(
                StoreManager.getOreQuantity()));
        muleQuantityLabel.setText(String.valueOf(
                StoreManager.getMuleQuantity()));
        foodPrice.setText(String.valueOf(
                StoreManager.getFoodPrice()));
        energyPrice.setText(String.valueOf(
                StoreManager.getEnergyPrice()));
        orePrice.setText(String.valueOf(
                StoreManager.getOrePrice()));
        ...
        ArrayList<ButtonEntry<ButtPackage,
                Label>> buttons =  new ArrayList<>();
        buttons.add(new ButtonEntry<>(
                new ButtPackage(foodUp, foodDown), enterFood));
        buttons.add(new ButtonEntry<>(
                new ButtPackage(energyUp, energyDown), enterEnergy));
        buttons.add(new ButtonEntry<>(
                new ButtPackage(oreUp, oreDown), enterOre));
        for (ButtonEntry<ButtPackage, Label> x : buttons) {
            ...
            plusButton.setOnAction((event) -> {
                    ...
                    int multiplier = 1;
                    int newNet = 0;
                    int oldNet = Integer.parseInt(
                            netGainLabel.getText());
                    int storeNum = Integer.parseInt(
                            storeItem.getText());
                    if (StoreManager.isBuy()) {
                        multiplier = -1;
                    }
                    //checks to see if sufficient funds/
                    // quantities in store/player inventory
                    //sets money label, item quantity label,
                    // and player inventory label
                    if (storeItem.getId().equals("enterFood")
                            && ((StoreManager.isBuy()
                            && Integer.parseInt(
                            foodQuantityLabel.getText()) - 1 >= 0)
                            || (StoreManager.isSell()
                            && Integer.parseInt(newFood.getText()) - 1 >= 0))
                            && Integer.parseInt(newMoney.getText())
                            + multiplier * StoreManager.getFoodPrice() >= 0) {
                        foodQuantityLabel.setText(String.valueOf(
                                Integer.parseInt(foodQuantityLabel.getText())
                                        + multiplier));
                        newFood.setText(String.valueOf((Integer.parseInt(
                                newFood.getText())) - multiplier));
                        newMoney.setText(String.valueOf(Integer.parseInt(
                                newMoney.getText()) + multiplier
                                * StoreManager.getFoodPrice()));
                        newNet = oldNet + multiplier
                                * StoreManager.getFoodPrice();
                        validEntry = true;
                    } else if (storeItem.getId().equals("enterEnergy")
                            && ((StoreManager.isBuy()
                            && Integer.parseInt(
                            energyQuantityLabel.getText()) - 1 >= 0)
                            ||  (StoreManager.isSell()
                            && Integer.parseInt(newEnergy.getText()) - 1 >= 0))
                            && Integer.parseInt(newMoney.getText())
                            + multiplier * StoreManager.getFoodPrice() >= 0) {
                        energyQuantityLabel.setText(String.valueOf(
                                Integer.parseInt(energyQuantityLabel.getText())
                                        + multiplier));
                        newEnergy.setText(String.valueOf(
                                (Integer.parseInt(newEnergy.getText()))
                                        - multiplier));
                        newMoney.setText(String.valueOf(
                                Integer.parseInt(newMoney.getText())
                                        + multiplier
                        ...
                        validEntry = true;
                            ...
                            && ((StoreManager.isBuy()
                            && Integer.parseInt(
                            oreQuantityLabel.getText()) - 1 >= 0)
                            ||  (StoreManager.isSell()
                            && Integer.parseInt(newOre.getText()) - 1 >= 0))
                            && Integer.parseInt(newMoney.getText())
                            + multiplier
                            * StoreManager.getFoodPrice() >= 0) {
                        oreQuantityLabel.setText(String.valueOf(
                                Integer.parseInt(
                                        oreQuantityLabel.getText())
                                        + multiplier));
                        newOre.setText(String.valueOf(
                                (Integer.parseInt(
                                        newOre.getText())) - multiplier));
                        newMoney.setText(String.valueOf(
                                Integer.parseInt(newMoney.getText())
                                        + multiplier
                                        * StoreManager.getOrePrice()));
                    ...
                    //sets netGain label if there are sufficient
                    // funds/quantities in store/player inventory
                    if (validEntry) {
                        if (newNet > 0) {
                            plusSign = "+";
            ...
            minusButton.setOnAction((event) -> {
                    ...
                    //sets money label, netGain label,
                    // store quantity label, and
                    // player inventory quantity label
                    if (storeNum > 0) {
                        if (storeItem.getId().equals("enterFood")) {
                            foodQuantityLabel.setText(String.valueOf(
                                    Integer.parseInt(
                                            foodQuantityLabel.getText())
                                            + multiplier));
                            newFood.setText(String.valueOf(
                                    (Integer.parseInt(
                                            newFood.getText())) - multiplier));
        ...
        muleCombo.setOnAction((event) -> {
                ...
                //selecting the first mule does not
                // require subtracting out previous mule prices
                ...
                if (!StoreManager.isBuy() && !StoreManager.isSell()) {
                    ...
                    netGain = Integer.parseInt(
                            netGainLabel.getText()) - previous + mulePrice;
                }
                ...
                if (!selected.equals("(No Mule)")) {
                    purchaseMule.setImage(new Image(
                            "MULE/View/Images/mule"
                                    + muleCombo.getSelectionModel()
                                    .getSelectedItem()
                                    + ".gif"));
        ...
        completeButton.setOnAction((event) -> {
                ...
                GameManager.getCurrentPlayer().setMoney(
                        Integer.parseInt(newMoney.getText()));
                GameManager.getCurrentPlayer().setFoodCount(
                        Integer.parseInt(newFood.getText()));
                GameManager.getCurrentPlayer().setEnergyCount(
                        Integer.parseInt(newEnergy.getText()));
                GameManager.getCurrentPlayer().setOreCount(
                        Integer.parseInt(newOre.getText()));
                StoreManager.setFoodQuantity(
                        Integer.parseInt(foodQuantityLabel.getText()));
                StoreManager.setEnergyQuantity(
                        Integer.parseInt(energyQuantityLabel.getText()));
                StoreManager.setOreQuantity(
                        Integer.parseInt(oreQuantityLabel.getText()));
                ...
                if (!muleCombo.isDisabled() && StoreManager.isBoughtMule()) {
                    StoreManager.setMuleQuantity(
                            Integer.parseInt(muleQuantityLabel.getText()) - 1);
                    Image image = new Image(
                            "MULE/View/Images/walkingCatMuleCursor.gif");
                    GameManager.getMapGrid().setCursor(new ImageCursor(image));
                    //GameManager.currentPlayer.getMules().add(
                    // new Mule((String) muleCombo.
                    // getSelectionModel().getSelectedItem()));
                    GameManager.getCurrentPlayer().setCurMule(
                            (String) muleCombo.
                                    getSelectionModel().getSelectedItem());
